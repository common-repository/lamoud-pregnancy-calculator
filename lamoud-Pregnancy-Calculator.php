...
define( 'lamoud-Pregnancy-Calculator', '1.0.2' );
add_action('wp_enqueue_scripts', 'lmpc_enqueue_custom_style');
function lmpc_enqueue_custom_style() {
    wp_enqueue_script('lmpc-js', plugin_dir_url( __FILE__ ) . 'includes/lamoud.js', array(), '1.0.2', true);
    wp_enqueue_style( 'lmpc-style', plugin_dir_url( __FILE__ ) . 'includes/lamoud.css', array(), '1.0.2', 'all' );
    if ( is_rtl() ) {
      wp_enqueue_style( 'lmpc-style-rtl', plugin_dir_url( __FILE__ ) . 'includes/lamoud-rtl.css', array(), '1.0.2', 'all' );
    }
  }
